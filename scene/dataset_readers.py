...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # add depth and normal image loading
        depth_image_path = os.path.join(images_folder, image_name+'_depth.npy')
        depth = np.load(depth_image_path)
        
        # mask loader, some results in replica has a very large depth and we ignore it
        try:
            mask_image_path = os.path.join(images_folder, image_name+'_mask.npy')
            mask = np.load(mask_image_path)
            mask = (mask > 0.5).astype(np.int)
        except:
            mask = np.ones_like(depth)
        
        normal_image_path = os.path.join(images_folder, image_name+'_normal.npy')
        normal = np.load(normal_image_path)
        # important that the normal data from omnidata is normalized
        normal = normal * 2 - 1
        
        
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height,
                              depth=depth, depth_path=depth_image_path, normal=normal, normal_path=normal_image_path,
                              mask=mask, mask_path=mask_image_path)

	...
	bool prefiltered)
{
	...
	float3 n_orig = { normal_precomp[3 * idx], normal_precomp[3 * idx + 1], normal_precomp[3 * idx + 2] };
	float3 n_view = transformVec4x3(n_orig, viewmatrix);// rotatie the normal to view space
	...
	normals[3 * idx + 0] = n_view.x;
	normals[3 * idx + 1] = n_view.y;
	normals[3 * idx + 2] = n_view.z;
	radii[idx] = my_radius;
	...
	float* __restrict__ out_median_depth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++){
				...
				N[ch] += normal[collected_id[j] * CHANNELS + ch] * alpha * T;
			}
			D += depths[collected_id[j]] * alpha * T;
			// add median depth inference
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++) {
			...
			// out_normal[ch * H * W + pix_id] = N[ch]/(1-T);
		}
		out_depth[pix_id] = D;

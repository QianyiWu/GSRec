...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None):
        ...
        self.start_model = args.start_model if args.start_model else args.model_path
        ...
        if self.loaded_iter:
            self.gaussians.load_ply_sparse_gaussian(os.path.join(self.start_model,
                                                           "point_cloud",
                                                           "iteration_" + str(self.loaded_iter),
                                                           "point_cloud.ply"))
            self.gaussians.load_mlp_checkpoints(os.path.join(self.start_model,
                                                           "point_cloud",
                                                           "iteration_" + str(self.loaded_iter),
                                                           "checkpoint.pth"))
        else:
